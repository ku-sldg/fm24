...
\begin{definition}[Adversary Event]
    An adversary event is any action performed by an adversary to change the behavior of a component in a target system. Adversary events are classified as either corruption or repair events. 
\end{definition}
...
\begin{definition}[Time-Constrained Adversary Event]
    A time-constrained adversary event is an adversary event where the attacker must perform the corruption or repair of a component in between measurement events thus limiting the time available to complete the action.
\end{definition}
...
\begin{definition}[Attack Graph Equivalence]
  Attack graphs $G$ and $H$ are equivalent if and only they are isomorphic.
\end{definition}
