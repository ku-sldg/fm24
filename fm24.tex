...
Properties of orderings and the Chase model finder developed by MITRE \cite{Ramsdell:2020:Chase} are paramount tools for our adversary-constrained protocol ordering and subsequent analysis. Key concepts include preorder and equivalence relations for Copland protocols as well as Chase generated attack graphs. These topics are explained in more detail in the following sections.

\subsection*{Properties of Orders}

Critical to our study is various relations $R$ between objects in a set $X$. We specifically consider the following properties, formally defined below: 
...
\noindent When attempting to introduce new ordering relations, as with this protocol ordering, each property of the ordering must be proven true in order to accurately state the ordering holds.  
...
Chase can be leveraged to understand the susceptibility of a Copland protocol to previously undetectable attacks \cite{Rowe:2021:AutomatedTrust}. When using Chase in this way, the model finder reasons over user-provided axiomatized system dependencies and assumptions to automatically produce output containing all possible protocol executions, rendered in \texttt{.xhtml} as attack trees. Within the attack trees, the presence of an active adversary can be recognized by visualizing corruption and repair events. A corruption event indicates an adversary has performed some activity to manipulate the behavior of a component such that it is in a malicious state yet, when measured, the component returns seemingly trustworthy evidence. A repair event indicates that an adversary has repaired the previously corrupted component by reverting it to its state prior to the corruption. Both corruption and repair events can be categorized as evidence tampering where the specific details of how an adversary performs tampering are not the focus of this work or the Chase work. Instead, the goal of Chase for protocol analysis is to produce easily identifiable corruption and repair events, allowing users to better understand potential vulnerabilities within the system and Copland protocol. 

%\citet{Rowe:2021:AutomatedTrust} leveraged the initial Chase reasoning to confirm that the composition of Copland protocols directly impacts the trustworthiness of produced evidence. When testing this hypothesis, authors proved it is possible for an adversary to manipulate components in an informed way such that the resulting evidence appears trustworthy but is actually fraudulent. Authors confirmed this hypothesis using Chase to automate the adversarial analysis. Given a theory describing the system configuration and other system-dependent assumptions, Chase allowed \citet{Rowe:2021:AutomatedTrust} to discover components that had been unknowingly manipulated by an adversary. 

%The development of Chase required \citet{Ramsdell:2020:Chase} to enrich the Copland event semantics to include corruption and repair events to denote adversary actions.

%In turn, this may provide opportunity to manipulate the protocol to diminish the likelihood of such vulnerabilities.

% Throughout the Chase analysis, it is assumed that an adversary may corrupt or repair measurements as warranted yet still pass appraisal. 

Input to the Chase model finder includes the Copland phrase, a model of Copland adversary events, a model of the evaluated protocol, and a Makefile. This information is captured respectively in the four files below with examples of each file in Appendix \ref{Appendix.Chase}.   

\vspace*{-10mm}

\begin{align*}
    \squash
    1. &\texttt{<basename>.cop} \\
    2. &\texttt{<basename>.gln} \\
    3. &\texttt{thy.gli} \\ 
    4. &\text{Makefile}
\end{align*}

Aside from the protocol evaluation model in \texttt{<basename>.gln}, files are straightforward and reusable across Chase examples. The analyzed Copland protocol is specified in \texttt{<basename>.cop} and must be written in the proper Copland grammar as the \texttt{.cop} extension triggers a Copland parser. In the theory file \texttt{thy.gli}, Chase maintainers include standard theory describing adversary-ordered executions where basic assumptions and axioms aligning with the Chase logic are indicated. This file may include minimality conditions, event ordering conditions, such as a strict partial order over events, rules for saturated queries, and anything else that may pertain to the circumstances. The Makefile specifies the rules to compile the theories to generate the \texttt{.xhtml} output. In our case, both the theory and Makefile are provided by Chase maintainers.

Theory in \texttt{<basename>.gln} is more complicated, requiring user-defined model finding parameters, axiomatizated system dependencies, and other contextual nuances. Within the first lines of this file, Chase input parameters including bound (limiting the size of the structure), limit (limiting the number of inference steps), and order of formula consideration (often the order at which they appear) are stated. The following line declares the measurement event where an adversary evades detection. This is often the final measurement event because generated evidence appears trustworthy when the system has actually been maliciously manipulated. System dependencies are also described in this file as logical statements. If applicable, the recent or deep assumptions \cite{Rowe:2016:Confining} can also be stated. The final lines contain file inputs, including the previously defined theory of adversary-ordered executions and the Copland protocol file. It is critical that this file accurately depict the system, its dependencies, and other assumptions, to produce correct output. 
%Because so much of this file is situationally dependent, it is difficult to reuse existing file. 

% system dependencies are stated with first order logic, specifically geometric logic. Within the theory input, many important assumptions are stated. That is, an analyst specifies the measurement events at which an adversary may go undetected. This is often the final measurement event although that claim need not be ubiquitous. 

A run of the Chase analysis tool utilizes the provided files to produce structures which satisfy the given theory. In the Copland protocol analysis context, these structures are automatically assembled into attack trees \cite{Rowe:2021:AutomatedTrust} and rendered as \texttt{.xhtml}.  Within the output, first statements include the problem configuration with the analyzed protocol, the protocol's abstract syntax tree, and the protocol's execution semantics. This configuration information is followed by all possible attack trees which satisfy the given model. These sequential attack trees \cite{Horne:Attack, Jhaware:attack} are formal models of the ways in which an adversary might attack a system. 
...
% \begin{figure}[hbtp]
%     \centering 
%     \input{examples/general-example/ast.tex}
%     \caption[Example Chase AST]{Example Chase generated AST}
%     \label{fig:example-ast}
% \end{figure}
...
% Considering the aforementioned system specifications and he following set of models may be generated, as shown in  Figure \ref{fig:chase-ex}. These models vaguely follow real life examples but are fabricated to showcase various patterns which may arise during analysis. In these figures, the prefix \emph{ms} represents measurement events, the prefix \emph{c\_} represents corruption events, and the prefix \emph{r\_} represents repair events. This labeling system mirrors the actual labeling system of the Chase model finder. 

%However, it is possible that once an adversary has performed a corruption event, it is easier to perform a repair event because the adversary has an understanding of the corrupted component.
...
%The Chase inputs may be manipulated in the following ways.... Things I tried but didn’t have much impact… 
%- Change measurement combination (parallel to sequence) 
%- Add measurements
...
We introduce a Coq-based, formally-verified mathematical model that considers the abilities of an active adversary to order protocols by attack difficulty. This ordering methodology compares two distinct Copland protocols by comparing their Chase-generated attack graphs. From the graphs, we reason over a protocol's hypothesized corruption and repair events, introducing additional definitions and orderings for adversary event comparisons. These event comparisons are foundational for our adversary-constrined protocol ordering, ultimately allowing us to link preorder and equivalence relations to Copland phrases.

%We leverage attack graph comparisons to ultimately introduce an adversary-constrained protocol ordering over Copland phrases. 

%first using Chase to generate all possible attacks and subsequently comparing if all attacks from a comparable protocol better confine the adversary. 

%We first compare the Chase generated attack graphs individually and then compare them as a set. The ordering decision over sets of attack graphs can be linked to the protocols which produced the graphs to assess if the change in the protocol better confined the adversary. 
%, to an ordering decision yielding the more confining protocol.  
...
%, as denoted with the ordering relation ($\leq$) above the arrow 
...
%the change in $P1$ and $P2$ is $\triangle{P}$ as seen with the top line. We can generate the set of attack graphs $\{ a1, a2, a3\}$ for $P1$ and the set of attack graphs for $\{a1', a2', a3'\}$ for $P2$. When comparing the individual Chase generated attack graphs, we see there exists some relation between $a1$ and $a1'$, $a2$ and $a2'$, and $a1$ and $a3'$ as represented with the dotted lines where the arrow points towards the equally difficult or more difficult attack. Looking at the sets of attack graphs, it is apparent that every graph in $P2$ has a related graph in $P1$ that is either the same or less difficult to attack. From this observation, we conclude that $P2$ is at least as difficult to attack thus at least as strong as $P1$. This logic is propagated to reveal that $\triangle{P}$ made the protocol stronger.  

%Less than or equal to is not the only operator we introduce for protocol comparison. 
